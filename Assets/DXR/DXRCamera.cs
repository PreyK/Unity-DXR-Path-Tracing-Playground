...
{	public Texture SkyboxTexture;
	...
	private void Update()
	{
		Shader.SetGlobalInt("_MaxRayBounce", RayBounces);
		//Vector3 l = directionalLight.transform.forward;
		//Shader.SetGlobalVector("_DirectionalLight", new Vector4(l.x, l.y, l.z, directionalLight.intensity));
		Shader.SetGlobalVector("_camera", transform.position);
		...
		{
			if(watched[i].hasChanged){
				watched[i].hasChanged = false;
				UpdateParameters();
			}
		}
	}
	...
	private void UpdateParameters()
	{
		...
		_rayTracingShader.SetTexture("_SkyboxTexture", SkyboxTexture);
		_rayTracingShader.SetFloat("_skyExposure", skyExposure);
